...
function printUsage() {
  console.log(
    "Usage: bun run scripts/generate-token.ts [readonly|readwrite] [userId]"
  );
  console.log("\nExamples:");
...
function generateToken() {
  ...
  try {
    ...
    console.log("\n🧪 Test with curl:");
    console.log(
      `curl -H "Authorization: Bearer ${token}" http://localhost:3000/v1/cache/test-hash`
    );
  } catch (error) {
...
function generateSimpleTokens() {
  ...
  const tokenPart = Buffer.from(JSON.stringify(tokenData)).toString(
    "base64url"
  );
